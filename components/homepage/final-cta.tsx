...
function PrimaryCTA({ label, href }: { label: string; href: string }) {
  return (
    <Link
      href={href}
      className="group relative inline-flex h-12 min-h-[48px] w-full items-center justify-center gap-2.5 overflow-hidden rounded-full bg-white px-8 text-sm font-semibold tracking-[-0.01em] text-[#131517] shadow-[0_1px_0_rgba(255,255,255,0.08)_inset] transition-[transform,box-shadow] duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-px hover:shadow-[0_12px_40px_rgba(255,255,255,0.14)] active:translate-y-0 active:shadow-[0_4px_16px_rgba(255,255,255,0.08)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#131517] sm:w-auto motion-reduce:hover:translate-y-0 motion-reduce:hover:shadow-[0_1px_0_rgba(255,255,255,0.08)_inset]"
    >
      <span
        aria-hidden="true"
      ...
      <ArrowRight
        size={16}
        aria-hidden="true"
        className="relative shrink-0 transition-transform duration-300 ease-out group-hover:translate-x-0.5 motion-reduce:group-hover:translate-x-0"
      />
    </Link>
  );
}
...
function SecondaryContactAction({ label, href }: { label: string; href: string }) {
  return (
    <Link
      href={href}
      className="group/link inline-flex min-h-11 w-full items-center justify-center gap-2 text-sm font-medium text-white/45 transition-colors duration-300 hover:text-white/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-[#131517] sm:w-auto"
    >
      <span>{label}</span>
      <ArrowRight
        size={15}
        aria-hidden="true"
        className="shrink-0 opacity-60 transition-[transform,opacity] duration-300 group-hover/link:translate-x-0.5 group-hover/link:opacity-100 motion-reduce:group-hover/link:translate-x-0"
      />
    </Link>
  );
}
...
export function FinalCTA() {
  ...
  return (
    <section
      ...
      style={{
        background:
          'linear-gradient(180deg, var(--landing-neutral) 0%, #1A1C20 22%, #131517 100%)',
      }}
    >
      <div
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 80% 60% at 50% 100%, rgba(255,255,255,0.035) 0%, transparent 65%)',
        }}
      />
